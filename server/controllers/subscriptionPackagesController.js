const SubscriptionPackages = require('../models/subscriptionPackagesModel');
...
exports.createSubscriptionPackage = async (req, res) => {
    try {
        ...
        const package= new SubscriptionPackages({
            title,
            description,
            price
        });

        await package.save(); // Save the discussion to the database

        return res.status(201).json({ message: "Subscription Package Created Successfully", data: package });
    } catch (error) {
